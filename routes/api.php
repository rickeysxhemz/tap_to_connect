...
use App\Http\Controllers\NfcBusinessLinkController;
...
Route::group(['prefix' => 'v1'], function() {
    Route::group(['middleware' => ['optionalAuth:sanctum', 'verified']], function () {
        ...
        Route::get("nfc-link-check/{url_code}", [NfcBusinessLinkController::class, 'checkLink']);
        Route::get("register-nfc/{url_code}", [NfcBusinessLinkController::class, 'registerNfc']);
        Route::get("digital-profiles", [NfcBusinessLinkController::class, 'digitalProfiles']);
        Route::get("digital-links", [NfcBusinessLinkController::class, 'digitalLinks']);
        Route::put("update-nfc-links/{id}", [NfcBusinessLinkController::class, 'updateNfcLinks']);
        Route::post("reset-nfc", [NfcBusinessLinkController::class, 'resetNfc']);
        Route::post("update-vcf", [BiolinkController::class, 'updateNfc']);
        Route::get("download-vcf/{id}", [BiolinkController::class, 'downloadVcf']);

        Route::post("nfc-links/csv/export", [CommonCsvExportController::class, 'exportNfcCards']);
        Route::post("nfc-links/csv/import", [NfcBusinessLinkController::class, 'importCsv']);

        Route::post("copy-biolink", [BiolinkController::class, 'copyBiolink']);
        Route::post("send-connect-mail", [BiolinkController::class, 'sendConnectMail']);

        Route::get("contacts", [BiolinkController::class, 'getContacts']);
        Route::get("download-contacts", [BiolinkController::class, 'downloadContacts']);
    });
});
